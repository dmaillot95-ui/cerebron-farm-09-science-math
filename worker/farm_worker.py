...
def build_payload(spec,prompt):
    ...
    for p in spec.get('parameters',[]):
        ...
        if lname in {'message','prompt','text','query','input','instruction','user_message'}:
            ...
            payload[name]='Rigorous mathematical research. CLAIM<=EVIDENCE.'
...
result={'role':role,'model':model,'family':family,'success':bool(ok),'inference_success':bool(ok),'status':'UNREVIEWED_EXTERNAL_AGENT_OUTPUT' if ok else 'EXTERNAL_INFERENCE_FAILED','output':text if ok else '','error':None if ok else meta.get('error'),'meta':meta}
Path('results').mkdir(exist_ok=True)
Path(f'results/{role}.json').write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8')
print(json.dumps({'role':role,'success':bool(ok),'family':family}))
